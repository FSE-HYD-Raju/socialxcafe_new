...
import ConfirmDeleteModal from "../common/DeleteCofirmation";
import MemberCheckInModal from "./MemberCheckInModal";
export default function AdminEvents() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "events"), async (snapshot) => {
      ...
      for (const event of list) {
        ...
        const baseDate = event.eventDate.toDate();
  ...
  const exportEvents = () => {
    const data = events.map((e) => ({
      Title: e.title,
      Date: e.eventDate?.toDate().toLocaleString(),
      Price: e.price,
      Capacity: e.capacity,
